...
class InserirTreino
{
    ...
    {
        $comando = $this->pdo->prepare("INSERT INTO treino (data_treino, tipo, exercicios, repeticao, serie, objetivo, duracao, frequencia, nome_treino) VALUES(:data_treino, :tipo, :exercicios, :repeticao, :serie, :objetivo, :duracao, :frequencia, :nome_treino)");
        $comando->bindValue(":data_treino", $data);
        $comando->bindValue(":tipo", $tipo);
        $comando->bindValue(":exercicios", $exercicios);
        $comando->bindValue(":repeticao", $repeticoes);
        $comando->bindValue(":serie", $series);
    ...
    {
        $resultado = array();
        $comando = $this->pdo->query("SELECT nome_treino,id_treino FROM treino");
